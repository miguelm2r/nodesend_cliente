...
export default function Dropzone() {
    ...
    const { mostrarAlerta, subirArchivo, crearEnlace, cargando } = useContext(appContext)

    const { usuario, autenticado } = useContext(authContext)

    const onDropRejected = () => {
        mostrarAlerta("No se pudo subir. El límite es 1MB")
    }
    ...
    const { getRootProps, getInputProps, isDragActive, acceptedFiles } = useDropzone({onDropAccepted, onDropRejected, maxSize: 100000})
            ...
            { acceptedFiles.length > 0 ? (
                <div className="mt-10 w-full">
                    ...
                    { cargando ? <p className="my-10 text-center text-gray-600">Subiendo Archivo</p> : (
                        <button
                            type="button"
                            className="bg-blue-700 w-full py-3 rounded-lg 
                            text-white my-10 hover:bg-blue-800"
                            onClick={() => crearEnlace()}
                        >
                            Crear Enlace
                        </button>
                            ...
                            <>
                                <div className="text-center">
                                    ...
                                    <button 
                                        ...
                                        text-white my-10 hover:bg-blue-800"
                                        type="button"
                                    >Selecciona archivo para subir</button>
                                </div>
                            </>
                        }
                </div>
